...
module Harpy
  ...
  module Spv
    ...
    def verify_inclusion(
      ...
      return false unless verify_header_chain(headers, trusted_genesis_hash, now)

      Merkle.verify_proof(txid, proof, headers.last.merkle_root)
    end
    ...
    def verify_anchor(
      ...
      return false unless verify_header_chain(headers, trusted_genesis_hash, now)
      return false if headers.last.anchor_root.empty?

      Merkle.verify_proof(record_hash, proof, headers.last.anchor_root)
    end
